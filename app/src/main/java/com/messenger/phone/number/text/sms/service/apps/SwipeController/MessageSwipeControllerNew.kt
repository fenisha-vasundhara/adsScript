...
class MessageSwipeControllerNew(private val context: Context, private val swipeControllerActions: SwipeControllerActions) :
    ...
    override fun getMovementFlags(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder): Int {
        ...
        isLeftSwipe = conlist[viewHolder.position].type == 2

//        return if (isLeftSwipe) {
//            ItemTouchHelper.Callback.makeMovementFlags(ACTION_STATE_IDLE, LEFT)
//        } else {
//
//        }
    ...
    private fun setTouchListener(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        recyclerView.setOnTouchListener { _, event ->
            ...
            if (swipeBack) {
                if (Math.abs(mView.translationX) >= this@MessageSwipeControllerNew.convertTodp(80)) {
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        if (startTracking) {
            if (!isVibrate && mView.translationX >= convertTodp(60)) {
        ...
        val x: Int = if (mView.translationX > convertTodp(65)) {
            convertTodp(65) / 2
        } else {
    ...
    private fun keepDateTextFixed(viewHolder: RecyclerView.ViewHolder) {
        val dateContainer = viewHolder.itemView.findViewById<View?>(R.id.datetxt_con) ?: return
        dateContainer.translationX = -viewHolder.itemView.translationX
    }
