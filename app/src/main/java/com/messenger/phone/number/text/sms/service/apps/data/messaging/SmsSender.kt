...
import com.messenger.phone.number.text.sms.service.apps.CommanClass.isSPlus
...
class SmsSender(val app: Application) {
    ...
    fun sendMessage(
        ...
        requireDeliveryReport: Boolean, messageUri: Uri, isgroupmessage: Boolean,
        messageid: Long = 0L, isfaildmessage: Boolean = false
    ) {
        ...
        sendInternal(
            subId, dest, messages, serviceCenter, requireDeliveryReport, messageUri, isgroupmessage,
            isfaildmessage = isfaildmessage, messageid = messageid
        )
    }
    ...
    private fun sendInternal2(
        ...
        if (com.simplemobiletools.commons.helpers.isSPlus()) {
    ...
    private fun sendInternal(
        ...
        Log.d("jigar", "sendInternal: <------------> $dest <----> ${messages.joinToString()} <------> $serviceCenter <------> ${messageUri.lastPathSegment!!.toLong()}")

        val insertedId = messageUri.lastPathSegment!!.toLong()
        val tredid = app.getThreadId(dest)
        val c: Date = Calendar.getInstance().time

        CoroutineScope(Dispatchers.IO).launch {
            Log.d("Opert", "isfaildmessage: <------------> 111 0")

            if (!isfaildmessage) {
                Log.d("Opert", "isfaildmessage: <------------> 111 1")

                conversation = Conversation(
                    0,
                    c.time.toString(),
                    false,
                    fetchContactIdFromPhoneNumber(dest, app)!!,
                    null,
                    false,
                    dest,
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ismessageid = singleton?.insertMessageToDatabase(
                        0, c.time.toString(), false,
                        fetchContactIdFromPhoneNumber(dest, app)!!,
                        null,
                        false,
                        dest,
                    ...
                    ismessageid?.let {
                        singleton?.updateMessageStatus("Sending", it)
                    }
                    Log.d("sendSMSManager", "setMessage:sendSMSManager <------------> isfaildmessage  8 ")

                }
            } else {
                Log.d("Opert", "isfaildmessage: <------------> 111 2")

                Log.d("sendSMSManager", "setMessage:sendSMSManager <------------> isfaildmessage  9")
                messageid?.let {
                    singleton?.updatemessagestatusfrommessageRepoApp("Sending", it)
                }
            }
        }
        ...
        try {
            if (sendMultipartSmsAsSeparateMessages) {
                ...
                Log.d("hello", "sendMessageCompat: <------------------------> 23")
                try {
                    smsManager.sendMultipartTextMessage(
                        dest, serviceCenter, messages, sentIntents, deliveryIntents
                    )
                } catch (E: Exception) {
                    Log.d("hello", "sendMessageCompat: <------------------------> 233")
                }
            }
        } catch (e: Exception) {
            Log.d("hello", "sendMessageCompat: <------------------------> 24")
            throw SmsException(ERROR_SENDING_MESSAGE, e)
        }
