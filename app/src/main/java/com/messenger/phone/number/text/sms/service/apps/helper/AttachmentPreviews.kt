...
import com.messenger.phone.number.text.sms.service.apps.CommanClass.isAudioMimeType
import com.messenger.phone.number.text.sms.service.apps.CommanClass.isCalendarMimeType
import com.messenger.phone.number.text.sms.service.apps.CommanClass.isPdfMimeType
import com.messenger.phone.number.text.sms.service.apps.CommanClass.isZipMimeType
...
import com.messenger.phone.number.text.sms.service.apps.databinding.ItemAttachmentVcardBinding
...
//    val textColor = context.getProperTextColor()
    val textColor = if (ThemeModeManager.shouldUseLightSystemBars(context)) {
        context.resources.getColor(R.color.black2, context.theme)
    } else {
        context.resources.getColor(R.color.white, context.theme)
    }
//    val primaryColor = context.resources.getColor(R.color.white, context.theme)

    filename.setTextColor(textColor)
    fileSize.setTextColor(textColor)

    icon.setImageResource(getIconResourceForMimeType(mimeType))
    val dialogBg = context.getDialogBackgroundColor()
    val dialogprimaryBg = context.getProperPrimaryColor()

    icon.background.setTint(dialogprimaryBg)
//    documentAttachmentHolder.backgroundTintList = ColorStateList.valueOf(dialogprimaryBg)

    documentAttachmentHolder.background = GradientDrawable().apply {
        shape = GradientDrawable.RECTANGLE
        cornerRadius = 16 * context.resources.displayMetrics.density
        setColor(dialogBg)
        setStroke((1 * context.resources.displayMetrics.density).toInt(), dialogprimaryBg)
    }
//    val background = root.background?.mutate()
//    if (background is GradientDrawable) {
//        background.setColor(dialogBg)
//    } else {
...
fun ItemAttachmentVcardBinding.setupVCardPreview(
    ...
    val dialogBg = activity.getDialogBackgroundColor()
...
//    vcardAttachmentHolder.backgroundTintList= ColorStateList.valueOf(primaryColor)

    vcardAttachmentHolder.background = GradientDrawable().apply {
        shape = GradientDrawable.RECTANGLE
        cornerRadius = 16 * context.resources.displayMetrics.density
        setColor(dialogBg)
        setStroke((1 * context.resources.displayMetrics.density).toInt(), primaryColor)
    }
