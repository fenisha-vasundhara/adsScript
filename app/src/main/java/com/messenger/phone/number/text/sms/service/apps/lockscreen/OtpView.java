...
public class OtpView extends AppCompatEditText {
  ...
  private boolean isSelectionUpdateInProgress = false;
  ...
  protected void onSelectionChanged(int selStart, int selEnd) {
    ...
    if (!isSelectionUpdateInProgress && getText() != null && selEnd != getText().length()) {
      moveSelectionToEnd();
    }
  }
  ...
  private void moveSelectionToEnd() {
    ...
    final int safeEnd = Math.max(0, text.length());
    if (getSelectionStart() == safeEnd && getSelectionEnd() == safeEnd) {
      return;
    }

    try {
      isSelectionUpdateInProgress = true;
      setSelection(safeEnd);
    } catch (IndexOutOfBoundsException ignored) {
      // Text layout/spans can be temporarily inconsistent while text is mutating.
    } finally {
