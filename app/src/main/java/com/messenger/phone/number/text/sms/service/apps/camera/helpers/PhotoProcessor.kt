...
import com.messenger.phone.number.text.sms.service.apps.camera.compensateDeviceRotation
import com.messenger.phone.number.text.sms.service.apps.camera.getOutputMediaFilePath
...
class PhotoProcessor(
    val activity: CameraActivity, val saveUri: Uri?, val deviceOrientation: Int, val previewRotation: Int, val isUsingFrontCamera: Boolean,
        ...
        try {
            ...
            if (activity.needsStupidWritePermissions(path)) {
                if (!activity.hasProperStoredTreeUri(activity.isPathOnOTG(path))) {
                    activity.toastMess("save_error_internal_storage")
            ...
            if (isUsingFrontCamera) {
                if (activity.config.flipPhotos || deviceRot != 0) {
                    ...
                    try {
                        ...
                        activity.toastMess(com.simplemobiletools.commons.R.string.out_of_memory_error)
                    }
                }
            }
