...
import com.messenger.phone.number.text.sms.service.apps.CommanClass.PERMISSION_READ_CONTACTS
...
import com.messenger.phone.number.text.sms.service.apps.CommanClass.getNameLetter
import com.messenger.phone.number.text.sms.service.apps.CommanClass.getStringValue
import com.messenger.phone.number.text.sms.service.apps.CommanClass.letterBackgroundColors
import com.messenger.phone.number.text.sms.service.apps.CommanClass.normalizePhoneNumber
import com.messenger.phone.number.text.sms.service.apps.CommanClass.queryCursor
...
class SimpleContactsHelper(val context: Context) {
    fun getAvailableContacts(favoritesOnly: Boolean, callback: (ArrayList<SimpleContact>) -> Unit) {
        ensureBackgroundThread {
            ...
            allContacts.forEach {
                ...
                if (name != null) {
                    ...
                    if (it.phoneNumbers.isNotEmpty()) {
                        it.name = it.phoneNumbers[0].normalizedNumber
                    }
                }
            ...
            allContacts = allContacts
                .filter { it.name.isNotEmpty() && it.phoneNumbers.isNotEmpty() }
                .distinctBy {
                    val firstPhoneNumber = it.phoneNumbers.firstOrNull()
                    if (firstPhoneNumber != null) {
                        val normalizedNumber = firstPhoneNumber.normalizedNumber ?: ""
                        val startIndex = maxOf(0, normalizedNumber.length - 9)
    ...
    private fun getContactPhoneNumbers(favoritesOnly: Boolean): ArrayList<SimpleContact> {
        val contacts = ArrayList<SimpleContact>()
        var exitnumberchack = ArrayList<PhoneNumber>()
        ...
        context.queryCursor(uri, projection, selection) { cursor ->
            ...
            if (!exitnumberchack.contains(phoneNumber)) {
                contacts.firstOrNull {
                    it.rawId == rawId

                }?.phoneNumbers?.add(phoneNumber)
                exitnumberchack.add(phoneNumber)
            }
        }
        return contacts
    }
    ...
    fun getContactLetterIcon(name: String): Bitmap {
        ...
        val size = context.resources.getDimension(com.intuit.sdp.R.dimen._10sdp).toInt()
