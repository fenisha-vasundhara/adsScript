package com.messenger.phone.number.text.sms.service.apps.Dialog.adapter
...
class FilepickerItemsAdapter(
    activity: Activity, val fileDirItems: List<FileDirItem>, recyclerView: MyRecyclerView,
    itemClick: (Any) -> Unit
...
//            Glide.with(activity).clear(holder.itemView.findViewById(R.id.list_item_icon)!!)
        }
    }

    private fun setupView(view: View, fileDirItem: FileDirItem) {
        val  list_item_name = view.findViewById<MyTextView>(R.id.list_item_name)
        val  list_item_details = view.findViewById<MyTextView>(R.id.list_item_details)
        ...
        view.apply {
            ...
            if (fileDirItem.isDirectory) {
                ...
                val placeholder = fileDrawables.getOrElse(
                    fileDirItem.name.substringAfterLast(".").lowercase(Locale.getDefault()), { fileDrawable })
